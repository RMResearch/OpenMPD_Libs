...
namespace GSPAT_BEM {
	/**
		...
		- NUM_POINTS_PER_TRAP: Integer, defining how many focus points we use to define a trap. Please note a top-bottom arrangement creates traps even if only one point is used (it creates standing waves). 
	...
	*/
	...
	/*_GSPAT_Export void setAlgorithm(GSPAT::Solution* solution, int numPointsPerTrap, int numIterations);

	_GSPAT_Export void setReflectorConfig(GSPAT::Solver* solver, int numMeshes, float* meshPositions, float* meshAreas, float* meshNormals, float* t2mMatrix);
	*/
